...
def create_merged_image_base(base_images, stylization_model, base_img_size=None, f_style_img=''):
    ...
    if len(args.styleImageOrgDir) != 0:
        ...
        if f_org_of_style_img is not None:
            ...
            org = org.resize((img_size, img_size), PIL.Image.Resampling.LANCZOS)
            ...
            print("Size of stylized image is ", tmp.size)
...
def main(f_style_img):
    ...
    stylized = stylized.resize((img_size, img_size), PIL.Image.Resampling.LANCZOS)
    ...
    for i, target_img in enumerate(os.listdir(target_path)):
        ...
        if args.outputMode != 2:
            input_img = input_img.resize(target_size, PIL.Image.Resampling.LANCZOS)
            ...
            if f_target_stylized_gt is not None:
                ...
                try:
                    ...
                    if target_gt_img.size != input_stylized.shape[0:2]:
                        target_gt_img = target_gt_img.resize(input_stylized.shape[0:2], PIL.Image.Resampling.LANCZOS)
